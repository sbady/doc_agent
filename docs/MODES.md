...
- `--release-parent` опционален (нужен только для авто-дополнения ключа проекта, если `release-target` передан числом).
...
## 7) Changelog preview: текст для исходного кода документации

- Что делает:
- читает существующую релизную таблицу в `TARGET_ISSUE_KEY` (или `--release-target`);
- строит готовый текст релизных заметок для вставки в исходный код документации;
- учитывает блоки `{% if domen == 'WCE' %}`/`{% else %}` и группировку по стендам;
- для WCE выделяет специальные группы по эмодзи в названии задачи:
  - только `🩲` -> портал `pro.t1.ru`;
  - только `👙` -> портал `Импульс`;
  - только `👽` -> конструктор форм;
  - если эмодзи несколько, задача идет в общий WCE-блок.
- Строки `не описываем` пропускаются.
- Команда:

```bash
python main.py --release-target MSP-8000 --mode changelog-preview
```
- Для `changelog-preview` указывайте полный ключ `release-target` (например, `MSP-8968`).

## 8) Refine: улучшение уже заполненной таблицы
